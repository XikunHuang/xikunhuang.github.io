...
对于分类问题, 我们的目标是基于有限的训练样本集尽可能准确地估计出后验概率$p(c\mid x)$, 而估计该后验概率有两种方式:

- 判别式模型: 直接建模$p(c \mid x)$.
- 生成式模型: 先对联合分布$p(x,c)$建模, 再由此得到$p(c \mid x)$.

本章着眼于生成式模型, 由贝叶斯定理可得 $$p(c\mid x)=\frac{p(x\mid c)p(c)}{p(x)}$$

所以问题关键就在于求得

- 类条件概率分布(class-conditional density)  $p(x\mid c)$
- 先验分布 $p(c)$
...
- 伯努利分布
  ...
  $$X \sim Ber(\theta)$$ $$Ber(x\mid \theta) = \theta^{\mathbb{I}(x=1)}(1-\theta)^{\mathbb{I}(x=0)}$$

  即随机变量$X \in \{0,1\}$, 且 $$p(X=1) = \theta$$ $$p(X=0) = 1 - \theta$$

- 二项分布

  $$X \sim Bin(n,\theta)$$  $$Bin(k\mid n,\theta) = \binom{n}{k} \theta^k (1-\theta)^{n-k} $$
...
- 多伯努利分布
    ...
    $$X \sim Cat(\theta)$$  $$Cat(x\mid \theta) = \prod_{j=1}^K \theta_j^{\mathbb{I}(x_j=1)}$$

    $X$取值为$K$维one-hot编码.

- 多项分布

    $$X \sim Mu(n, \theta)$$  $$Mu(x\mid n,\theta) = \binom{n}{x_1 \dots x_K} \prod_{j=1}^K \theta_j^k$$
...
- 贝塔分布
    ...
    $$X \sim Beta(a,b)$$  $$Beta(x\mid a,b) = \frac{1}{B(a,b)}x^{a-1}(1-x)^{b-1}$$
...
- 狄利克雷分布
    ...
    $$X \sim Dir(\alpha_1,\dots, \alpha_k)$$  $$Dir(x\mid \alpha_1,\dots,\alpha_k) = \frac{1}{B(\alpha_1,\dots,\alpha_k)}\prod_{k=1}^K x_k^{\alpha_k - 1} \mathbb{I}(x\in S_K)$$
...
下面从概率角度解释一下.
我们假定看到的数字是从概念$C$中均匀随机选取的. 我们引入似然 $p(\mathcal{D}\mid h)$, 则

$$p(\{2\}\mid h_{2^n}) = \frac{1}{6} $$

因为在$[1,100]$整数中只有$6$个整数满足$2^n$. 而

$$p(\{2\}\mid h_{even}) = \frac{1}{50}$$

同理可以计算$$p(\{2,4,8,16,32\}\mid h_{2^n}) \gg p(\{2,4,8,16,32\}\mid h_{even})$$
...
$$\hat{h}^{MAP} = \arg\max_h \ \ p(h\mid \mathcal{D})$$
由贝叶斯公式,上式可以写成
$$\hat{h}^{MAP} = \arg\max_h \ \ p(\mathcal{D}\mid h)p(h) = \arg\max_h \ \ [\log p(\mathcal{D}\mid h) + \log p(h)] $$
似然项会变(指数依赖样本大小$N$), 而先验项保持不变. 当数据量变大时, MAP趋向于**最大似然估计(MLE)**.
$$\hat{h}^{MLE} = \arg\max_h \ \ p(\mathcal{D}\mid h) = \arg\max_h \ \ \log p(\mathcal{D}\mid h)$$
...
这种情形下的后验预测分布为
$$p(\tilde{x} \in C \mid  \mathcal{D}) = \sum_h p(y=1\mid \tilde{x},h) p(h\mid \mathcal{D})$$
...
$$p(\tilde{x} \in C \mid  \mathcal{D}) = p(\tilde{x}\mid \hat{h}^{MAP})$$
...
后验概率分布为
$$p(\theta\mid \mathcal{D}) \propto Bin(N_1\mid \theta, N_0+N_1)\ Beta(\theta\mid a,b) \propto Beta(\theta\mid N_1+a, N_0+b)$$
...
$$\mathbb{E}[\theta\mid \mathcal{D}] = \lambda \frac{a}{a+b} + (1-\lambda)\hat{\theta}_{MLE}$$
也就是说, 后验是我们观察数据之前相信的知识和数据想要告诉我们的知识的一个折中.

### 后验预测分布

当我们有了后验概率分布$Beta(c,d)$后, 我们要预测下一次掷硬币是正面的概率.
$$p(\tilde{x}=1\mid \mathcal{D}) = \int_0^1 p(x=1\mid \theta)p(\theta\mid \mathcal{D})d\theta$$
即把$\theta$当成随机变量看待, 对$\theta$所有可能情况进行汇总.
带入 $p(x=1\mid \theta)=\theta$, $p(\theta\mid \mathcal{D})=Beta(\theta\mid c,d)$可得
$$p(\tilde{x}=1\mid \mathcal{D}) = \int_0^1\theta Beta(\theta\mid c,d) d\theta = \mathbb{E}[\theta\mid \mathcal{D}] = \frac{c}{c+d}$$
即在这个例子中我们有 $$p(\tilde{x}\mid \mathcal{D})=Ber(\tilde{x}\mid \mathbb{E}[\theta\mid \mathcal{D}])$$

#### 过拟合问题

如果我们在预测时不使用上面的BMA, 而是使用
$$p(\tilde{x}\mid \mathcal{D})=Ber(\tilde{x}\mid \hat{\theta}_{MLE})$$
即
$$p(\tilde{x}=1\mid \mathcal{D}) = \hat{\theta}_{MLE} = \frac{N_1}{N_0+N_1} $$
在数据集比较小时, 这非常容易过拟合, 比如样本集合为$3$次观测结果,且全是反面, 那么根据最大似然估计
$$p(\tilde{x}=1\mid \mathcal{D}) = \hat{\theta}_{MLE} = \frac{N_1}{N_0+N_1} = 0 $$
而贝叶斯方法可以很好的克服这个问题, 比如我们假定先验分布为均匀分布$a=b=1$, 那么根据后验预测分布我们有
$$p(\tilde{x}=1\mid \mathcal{D}) = \mathbb{E}[\theta\mid \mathcal{D}] = \frac{c}{c+d} = \frac{N_1+1}{N_0+N_1+2} = \frac{1}{5}$$
这正对应着**加一平滑**这种技巧.
...
假设数据 iid, 那么似然为
$$p(\mathcal{D}\mid \theta) = \prod_{i=1}^K \theta_k^{N_k}$$
实际上是一个多项分布.

### 先验

选取先验时我们主要考虑两点, 一是参数$\theta$取值范围, 另一个是希望先验对于似然来说是共轭先验. Dirichlet分布满足这两个条件.
$$Dir(\theta\mid \alpha_1,\dots,\alpha_K) = \frac{1}{B(\alpha_1,\dots,\alpha_K)}\prod_{k=1}^K \theta_k^ {\alpha_k-1}\mathbb{I}(\theta \in S_K)$$

### 后验

后验概率分布依然是Dirichlet分布
$$p(\theta\mid \mathcal{D}) \propto p(\mathcal{D}\mid \theta)p(\theta) = Dir(\theta \mid  \alpha_1+N_1,\dots, \alpha_K+N_K)$$
...
$$
\begin{eqnarray}
p(X=j\mid \mathcal{D}) &=& \int p(X=j\mid \theta)p(\theta\mid \mathcal{D})d\theta \\
...
$$p(Y\mid X,\mathcal{D})$$
我们可以利用贝叶斯公式,分别求出$p(X\mid Y,\mathcal{D})$和$p(Y)$. 但是求解$p(X\mid Y,\mathcal{D})$复杂度太高, 比如假设$X$的每一维取值范围为离散值,且大小为$S$, 那么$p(X\mid Y,\mathcal{D})$的参数个数将达到$KS^D$. 为了解决这个问题,朴素贝叶斯做了一个很强的假设:在给定类别的条件下, 特征之间相互独立. 即
$$p(X\mid Y=c) = \prod_{i=1}^D p(X^{(i)}\mid Y=c)$$
...
$$
\begin{eqnarray}
p(X_i,Y_i\mid \vec{\theta}) &=& p(Y_i\mid \vec{\theta})p(X_i\mid Y_i,\vec{\theta}) \\
&=& p(Y_i\mid \vec{\theta}) \prod_{j=1}^{D} p(X_i^{(j)}\mid Y_i,\vec{\theta})
\end{eqnarray}
$$
其中$\vec{\theta} = \{\pi_1,\dots,\pi_K, \vec{\theta}_{11},\dots,\vec{\theta}_{DK}\}$, 这里$\pi_i$是针对$p(Y=i)$的参数,$\vec{\theta}_{jc}$是针对$p(X^{(j)}\mid Y=c)$的参数. $\vec{\theta}_{jc}$取决于第$j$维特征选取什么样的分布.
对于数据集$\mathcal{D}=\{(X_1,Y_1),\dots, (X_n,Y_n)\}$, 可计算似然函数为
$$
\begin{eqnarray}
p(\mathcal{D}\mid \theta) &=& \prod_{i=1}^n p(Y_i\mid \theta) \prod_{i=1}^n\prod_{j=1}^D  p(X_i^{(j)}\mid Y_i,\theta) \\
&=& \prod_{i=1}^n p(Y_i\mid \theta) \prod_{i=1}^n\prod_{j=1}^D  \prod_{c=1}^K p(X_i^{(j)}\mid \theta_{jc})^{\mathbb{I}(Y_i=c)}
\end{eqnarray}
$$
求对数可得
$$\log p(\mathcal{D}\mid \theta) = \sum_{c=1}^K N_c\log \pi_c + \sum_{j=1}^{D}\sum_{c=1}^K\sum_{i: Y_i=c}p(X_i^{(j)}\mid \theta_{jc})$$

在求解$\vec{\pi}$时, 需要带入约束$\sum\limits_{c=1}^K\pi_c=1$, 利用拉格朗日乘子法可得
$$\pi_c^{MLE} = \frac{N_c}{N_1+\dots+N_K}$$
而$\vec{\theta_{jc}}$需要知道特征分布的具体形式才可以求.
...
$$p(\theta\mid \mathcal{D}) = p(\pi\mid \mathcal{D})\prod_{j=1}^D\prod_{c=1}^Kp(\theta_{jc}\mid \mathcal{D})$$

#### Using the model for prediction

最后的目标是用模型进行预测, 即计算
$$p(y=c\mid x,\mathcal{D}) \propto p(y=c\mid \mathcal{D})\prod_{j=1}^Dp(x_j\mid y=c_j,\mathcal{D})$$
按照Bayes的方法, 我们需要计算
$$p(y=c\mid x,\mathcal{D}) \propto [\int Cat(y=c\mid \pi) p(\pi\mid \mathcal{D})d\pi] \prod_{j=1}^D\int Ber(x_j\mid y=c,\vec{\theta_{jc}})p(\vec{\theta_{jc}}\mid \mathcal{D})d\vec{\theta_{jc}}$$
